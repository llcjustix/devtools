...
local function validate_room_access(room, room_name, user_jid, jwt_token)
    ...
    if code == 200 and response_body and #response_body > 0 then
        ...
        if ok and data then
            ...
            validation_result.requireLobby = data.requireLobby == true
            validation_result.isModerator = data.isModerator == true
            ...
            module:log("info", "Validation response: allowed=%s, reason=%s, requireAuth=%s, requireLobby=%s, isModerator=%s, status=%s",
                tostring(validation_result.allowed),
                tostring(validation_result.reason),
                tostring(validation_result.requireAuth),
                tostring(validation_result.requireLobby),
                tostring(validation_result.isModerator),
...
module:hook("muc-occupant-pre-join", function(event)
    ...
    if result.allowed then
        module:log("info", "✓ Access GRANTED: room=%s, user=%s, requireLobby=%s, isModerator=%s",
            room_name, user_jid, tostring(result.requireLobby), tostring(result.isModerator))

        -- Store moderator status in session for mod_jitsi_webhooks_enhanced to use
        if result.isModerator then
            session.jitsi_is_moderator = true
            module:log("info", "User %s is a moderator for room %s", user_jid, room_name)
        end

        -- If requireLobby is true, user should be placed in lobby
        if result.requireLobby then
            -- Store lobby requirement for mod_jitsi_webhooks_enhanced to handle
            room._data.lobby_required = true
            module:log("info", "Lobby enabled for room %s - user %s will wait for moderator", room_name, user_jid)
        end
